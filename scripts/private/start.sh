...
echo Building the *private* Docker container, SSHing into VM to grab Jupyter auth token
...
scp -i $PRIVATE_KEY_PATH -o UserKnownHostsFile=/dev/null -o CheckHostIP=no -o StrictHostKeyChecking=no $USER@$IP:~/token.txt $THIS_DIR_PATH/data/private/token.txt
